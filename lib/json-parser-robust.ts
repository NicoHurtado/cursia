...
export function parseAIJsonRobust(jsonString: string): any {
  ...
  const cleaned = cleanBasicJson(jsonString);
  ...
  for (let i = 0; i < strategies.length; i++) {
    try {
      console.log(
        `🔄 Trying JSON parse strategy ${i + 1}/${strategies.length}...`
      );
  ...
  throw new Error(
    `Failed to parse JSON after ${strategies.length} attempts: ${lastError?.message}`
  );
}
...
export function repairContentDocument(doc: any): any {
  ...
  doc.blocks = doc.blocks.filter((block: any) => {
    return block && typeof block === 'object' && block.type && block.data;
  });
