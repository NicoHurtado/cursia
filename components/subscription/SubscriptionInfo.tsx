...
export default function SubscriptionInfo({ subscription, onCancel, showCancelButton = true }: SubscriptionInfoProps) {
  const [isCancelling, setIsCancelling] = useState(false);
  const { toast } = useToast();

  const handleCancelSubscription = async () => {
    if (!confirm('¿Estás seguro de que quieres cancelar tu suscripción? Mantendrás acceso hasta el final del período pagado, pero no se renovará automáticamente.')) {
      return;
    }
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium">{PLAN_NAMES[subscription.plan as keyof typeof PLAN_NAMES]}</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium">{formatAmount(subscription.amountInCents, subscription.currency)}</span>
          </div>
        </div>
         ...
         {subscription.status === 'ACTIVE' && showCancelButton && (
           <div className="pt-4 border-t">
             <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
               ...
               <span>Al cancelar tu suscripción, mantendrás acceso hasta el final del período pagado.</span>
             </div>
             <Button
               variant="destructive"
        ...
        {subscription.status === 'CANCELLED' && (
          <div className="pt-4 border-t">
            <div className="text-center text-sm text-muted-foreground">
              ...
              <p>Mantienes acceso a todos los beneficios hasta el final del período pagado.</p>
              <p>Puedes reactivarla en cualquier momento desde la página de planes.</p>
            </div>
          </div>
        )}
      </CardContent>
