...
import { ArrowRight, ArrowLeft, BookOpen, Target, User, Loader2 } from 'lucide-react';
...
const levelOptions = [
  { value: 'BEGINNER', label: 'Principiante', description: 'Nuevo en este tema' },
  { value: 'INTERMEDIATE', label: 'Intermedio', description: 'Tengo conocimientos básicos' },
  { value: 'ADVANCED', label: 'Avanzado', description: 'Tengo experiencia en el área' },
];

const interestOptions = [
  'Tecnología', 'Marketing', 'Diseño', 'Negocios', 'Ciencias', 'Arte',
  'Idiomas', 'Salud', 'Deportes', 'Cocina', 'Música', 'Fotografía'
];

export function CourseFunnelModal({ isOpen, onClose, initialPrompt = '' }: CourseFunnelModalProps) {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();
  
  // Determine initial step based on whether we have a prompt
  const initialStep = initialPrompt.trim() ? 'preferences' : 'description';
  ...
  useEffect(() => {
    if (isOpen) {
      const newInitialStep = initialPrompt.trim() ? 'preferences' : 'description';
  ...
  const handleBack = () => {
    if (step === 'preferences') {
      // If we started with preferences (had initial prompt), close modal
      // If we started with description, go back to description
  ...
  const handleCreateAccountAndCourse = async () => {
    ...
    try {
      ...
      const mappedLevel = levelMapping[courseData.level as keyof typeof levelMapping];
      ...
      while (retryCount <= maxRetries) {
        ...
        if (retryCount <= maxRetries) {
          console.log(`Retrying course creation (attempt ${retryCount + 1}/${maxRetries + 1})`);
      ...
      if (!courseResponse.ok) {
        ...
        const errorMessage = errorData?.error || `Course creation failed (${courseResponse.status}: ${courseResponse.statusText})`;
        ...
        onClose();
        router.push('/dashboard/create-course?prompt=' + encodeURIComponent(courseData.description));
        return;
      }
  ...
  const getStepIcon = (stepName: FunnelStep) => {
    switch (stepName) {
      case 'description': return BookOpen;
      case 'preferences': return Target;
      case 'signup': return User;
      case 'creating': return Loader2;
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-center mb-6">
          {(['description', 'preferences', 'signup'] as FunnelStep[]).map((stepName, index) => {
            const StepIcon = getStepIcon(stepName);
            const isActive = step === stepName;
            const isCompleted = ['description', 'preferences', 'signup'].indexOf(step) > index;
        ...
        <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid gap-3 mt-3">
                  {levelOptions.map((option) => (
                    <div
                      key={option.value}
                      onClick={() => setCourseData(prev => ({ ...prev, level: option.value }))}
                      className={cn(
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCourseData(prev => ({ ...prev, username: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCourseData(prev => ({ ...prev, email: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCourseData(prev => ({ ...prev, password: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCourseData(prev => ({ ...prev, confirmPassword: e.target.value }))}
                  placeholder="Confirma tu contraseña"
                />
                {courseData.password && courseData.confirmPassword && courseData.password !== courseData.confirmPassword && (
          ...
          {step === 'creating' && (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium mb-2">Creando tu cuenta y curso...</h3>
              <p className="text-muted-foreground">
