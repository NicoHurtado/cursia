...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!courseId || !rating) {
      return NextResponse.json({ error: 'ID del curso y calificación requeridos' }, { status: 400 });
    }
    ...
    if (course.userId === session.user.id) {
      return NextResponse.json({ error: 'No puedes calificar tu propio curso' }, { status: 400 });
    }
